 (function () {
    ...
    var hist = document.querySelector(".HistText");
    var inverse = document.querySelector(".btn-inverse");
    var isin = document.querySelector(".btn-sin-i");
    var icos = document.querySelector(".btn-cos-i");
    var itan = document.querySelector(".btn-tan-i");
    var power = document.querySelector(".btn-p");
    ...
    equal.addEventListener("click",function(e){
        ...
        History.innerHTML.includes("*"))  {
            ...
            /*if the screen contains the symbol ^ if yes then extracting the string in the variable values
               converting the the array into an array containing only numbers then taking the values from the array and 
               doing the math.pow operations(works for only finding the answers for x^y and not xy^z or x^yz ) */

        } else if (screen.value.includes("^")){             
                    let myFunc = num => Number(num);              
                    var values = screen.value.match(/[-+]?[0-9]*\.?[0-9]+/g)
                    var array = Array.from(String(values),myFunc);
                       function numbersonly(value) {
                          if (typeof (value) === 'number'){
                          return value ;
                       }
                   }
                   var numbers =array.filter(numbersonly);
                   console.log(numbers)
                   x = numbers[0];
                   y = numbers[1];
